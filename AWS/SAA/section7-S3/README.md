...
- 사용 사례
  - 기존에 암호화되지 않은 Amazon S3 객체를 암호화
  - 버킷에 있는 기존객체를 다른 버킷으로 복사
  - 잠금을 설정하거나 기존 보존을 연장하거나 한 번에 최대 수십억 개의 객체에 대한 법적 보류를 활성화 또는 제거
  - 모든 객체의 태그(객체에 대한 메타데이터를 저장하는데 사용할 수 있는 문자열의 키-값 쌍)을 교체

## S3 스토리지 분석
### S3 인벤토리
- 사용자가 정의한 일정에 따라 객체 및 메타데이터의 플랫 파일 목록을 제공
- 객체 및 해당 메타데이터에 대한 목록을 CSV, Apache Optimized Row Columnar(ORC), Apache Parquet(Parquet)형식으로 제공
- 비즈니스, 규정 준수 및 규제 요건에 대한 객체의 복제 및 암호화 상태를 감사하고 보고할 수 있음

## S3 Storage Lens
- 객체 스토리지 사용 및 활동에 대한 분석 기능
- 지표를 분석해 스토리지 비용을 최적화하고 데이터 보호에 대한 모법 사례를 적용하는데 사용할 수 있는 상황별 권장 사항을 제공

## S3 액세스 포인트
- 공유 버킷에 대한 액세스 문제점
  - 여러 애플리케이션, 팀 및 개인 등 많은 사용자가 사용하는 공유 버킷에 대한 액세스 관리
  - 여러 권한 수준을 사용하는 수십 개에서 수백 개에 이르는 애플리케이션에 대한 액세스를 제어하는 단일 버킷 정책이 필요
  - 애플리케이션이 더 커지면 버킷 정책은 더 복잡해지고 관리하는 시간이 걸리며 변경으로 인해 다른 애플리케이션에 미치는 영향이 없는지 확인하기 위해 감사가 필요

## S3 액세스 포인트 사용
- S3 공유 버킷에 연결하기 위한 고유한 액세스 포인트 정책을 가지는 네트워크 엔드포인트
- 각 애플리케이션에 맞게 사용자 지정 이름과 권한으로 개별화된 액세스 포인트를 여러 개 생성 가능
- 인터넷이 아닌 Virtual Private Cloud(VPC)의 쵸엄나 수락하도록 액세스 포인트를 구성하여 프라이빗 네트워크에 대한 Amazon S3 데이터 액세스를 제한 가능
- 특정한 그룹, 사용자, 애플리케이션이 허용된 S3 데이터만 액세스 할 수 있게 가능
- 예, 재무팀 사용자는 재무관련 S3 데이터만 액세스하여 업로드, 수정, 삭제 가능

## S3 다중 리전 액세스 포인트
- 여러 리전의 버킷간의 자동 라우팅을하는 액세스 포인트
- 애플리케이션이 여러 AWS 리전에 있는 S3 버킷의 요청을 이행하는데 사용할 수 있는 글로벌 엔드 포인트를 제공
- 단일 리전에서 사용하는 것과 같이 하나의 액세스 포인트를 사용
- AWS 글로벌 네트워크를 통해 라우팅 상태가 액티브인 가장 가까운 S3 버킷으로 자동 라우팅

## S3 기타 기능
### S3 select & S3 Glacier Select
  - SQL 구문을 사용해 S3 객체의 콘텐츠를 필터링하여 원하는 데이터만 가져오는 기능
  - CSV,JSON, 또는 Apache Parquet 형식으로 저장된 객체만 검색 가능
...
###  요청자 지불 버킷(Requested Pays)
  ...
  - 요청자가 데이터 전송 및 요청에 대한 요금을 지불하고, 버킷 소유자는 데이터 저장 요금을 지불
...
- S3 버킷을 로컬 파일 시스템에 마운트 하여 파일 스토리지 처럼 사용 가능
- EC2, ECS, EKS 등의 AWS 서버 및 온-프레미스 리눅스 운영체제에서 지원
- 기존 파일 읽기, 새 파일 저장 가능, 기존 파일을 수정하거나 디렉터리 삭제는 안됨