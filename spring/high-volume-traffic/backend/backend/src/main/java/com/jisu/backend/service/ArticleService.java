...
public class ArticleService {
    ...
    public Article articleArticle(Long boardId, WriteArticleDto dto) {
        Board board = boardRepository.findById(boardId).orElseThrow(() -> new ResourceNotFoundException("게시판이 존재하지 않습니다."));
    ...
    public List<Article> getOldArticle(Long boardId, Long articleId) {
        return articleRepository.findTop10ByBoardIdAndIdLessThanOrderByCreatedDateDesc(boardId, articleId);
    }

    public List<Article> getNewArticle(Long boardId, Long articleId) {
        return articleRepository.findTop10ByBoardIdAndIdGreaterThanOrderByCreatedDateDesc(boardId, articleId);
    }
}
